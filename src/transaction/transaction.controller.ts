...
export class TransactionController {
  ...
  @ApiProperty({ title: 'Get Account Transactions', description: 'Get all transactions for a specific account with filtering and pagination' })
  ...
  @ApiQuery({ name: 'startDate', type: String, required: false, example: '2026-01-01', description: 'Start date (YYYY-MM-DD)' })
  @ApiQuery({ name: 'endDate', type: String, required: false, example: '2026-01-31', description: 'End date (YYYY-MM-DD)' })
  @ApiQuery({ name: 'search', type: String, required: false, description: 'Search in description, merchant, or reference' })
  @SwaggerResponse({ status: 200, description: 'Transactions retrieved successfully', schema: {
    example: {
  ...
  async getAccountTransactions(
    ...
    const cacheKey = this.getCacheKey(user.id, 'account', accountId, page, limit, type, status, category, startDate, endDate, search);
  ...
  @SwaggerResponse({ status: 401, description: 'Unauthorized - Invalid or missing token' })
  async getTransactionById(@CurrentUser() user: User, @Param('id') id: string): Promise<ApiResponse<TransactionSelect>> {
  ...
  @Roles(...Object.values(UserRole))
  @Post('deposit')
  @ApiProperty({ title: 'Deposit Cash', description: 'Deposit cash into account (simulates branch/ATM deposit)', type: DepositDto })
  @SwaggerResponse({ status: 201, description: 'Cash deposit completed successfully', schema: {
    example: {
  ...
  @Roles(...Object.values(UserRole))
  @Post('transfer')
  @ApiProperty({ title: 'Create Transfer', description: 'Transfer money between two accounts owned by the user', type: TransferDto })
  @SwaggerResponse({ status: 201, description: 'Transfer completed successfully', schema: {
    example: {
  ...
  @SwaggerResponse({ status: 400, description: 'Bad request - Insufficient balance, same account, or inactive account' })
